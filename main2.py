...
# === Agents ===
# Note: DQNAgent expects obs_size (without message), QNetwork adds +1 for message internally
agent_obs_sizes = {"agent_0": obs_size // 2, "agent_1": obs_size, "agent_2": obs_size}
agents = {a: DQNAgent(agent_obs_sizes[a], n_actions) for a in env.possible_agents}

def modify_obs(agent, obs):
    return obs[:agent_obs_sizes[agent]] if agent == "agent_0" else obs
...
for ep in range(episodes):
    ...
    for agent_idx, agent in enumerate(env.agent_iter()):
        ...
        if done:
            ...
            action = agents[agent].act(obs, epsilon, message=message)
        ...
        if not done:
            # Pass the received message to the remember function
            agents[agent].remember(last_obs[agent], action, reward, obs, done, message)
            last_obs[agent] = obs
            total_rewards[agent] += reward
            # Send message to next agent (for demonstration, use reward as message)
            messages[agent] = reward

        # Write each frame to global video
        frame = env.render()
...
# === Close writer and save reward plot ===
video_writer.release()
print("[INFO] Full training video saved to videos/full_training.mp4")
